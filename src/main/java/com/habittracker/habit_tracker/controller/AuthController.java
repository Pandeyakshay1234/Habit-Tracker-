...
public class AuthController {
     ...
     * @return ResponseEntity with AuthResponseDto (JWT + user profile) and HTTP 201 Created
     */
    ...
    public ResponseEntity<AuthResponseDto> register(
            ...
            @RequestBody                        // Deserializes incoming JSON payload into RegisterRequestDto record
            RegisterRequestDto request
    ) {
        // Delegate user creation and token generation to AuthService
        AuthResponseDto response = authService.register(request);

        // Return HTTP 201 Created status with AuthResponseDto JSON body
     ...
     * @return ResponseEntity with AuthResponseDto (fresh JWT + user profile) and HTTP 200 OK
     */
    ...
    public ResponseEntity<AuthResponseDto> login(
            ...
            @RequestBody                        // Deserializes incoming JSON payload into LoginRequestDto record
            LoginRequestDto request
    ) {
        // Delegate credential authentication and token generation to AuthService
        AuthResponseDto response = authService.login(request);

        // Return HTTP 200 OK status with AuthResponseDto JSON body
